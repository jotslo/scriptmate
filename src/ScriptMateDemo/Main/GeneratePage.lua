...
function module.SetupMenu(localPlugin, newData, searchContent)
	...
	for _, category in data.Categories do
		...
		if category.Title ~= "Welcome!" and category.Title ~= "Episode 1" then
			episode.ImageTransparency = 0.5
			episode.Title.TextTransparency = 0.5
			episode.Subtitle.TextTransparency = 0.5
			episode.Lock.Visible = true
		end
