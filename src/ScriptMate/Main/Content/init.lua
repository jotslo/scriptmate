...
local function grabMacros()
    local macros = {}

    for _, macro in script.Macros:GetChildren() do
        if not macro:IsA("LuaSourceContainer") then
            continue
        end

        local module = require(macro) or {}

        for _, content in module do
            macros[#macros + 1] = content
        end
    end
