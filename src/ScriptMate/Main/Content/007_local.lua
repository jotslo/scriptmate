...
module.Content = {
    ...
    },
    {
        ...
        Solution = [[-- require the module
local myModule = require(game.ReplicatedStorage.MyModule)

-- print the response
print(myModule)]],
        ExtraCode = [[
            local ms = game.ReplicatedStorage:FindFirstChild("MyModule")
            if not ms then
                ms = Instance.new("ModuleScript", game.ReplicatedStorage)
                ms.Name = "MyModule"
            end

            ms.Source = "return 'Hello'"
        ]]
    },
    {
        Type = "Exercise",
...
print(myModule[2])]],
        ExtraCode = [[
            ...
            ms.Source = '-- return the table of admins\nreturn {"Josh", "Bob", "Joe"}'
        ]]
    },
    {
        Type = "Exercise",
